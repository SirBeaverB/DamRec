...
配置: dataset=ml-1m, L=128, epochs=150, worker=4

用法: python scripts/run_non_streaming_experiments_1m.py
"""
...
def main():
    dataset = "ml-1m"
    max_seq_len = 128   # 用户历史更长，L=128
    epochs = 150        # 数据量大，提高 epoch 上限
    worker = 4          # 数据量大，多进程加载
    show_progress = True
    saved = False
    ...
    out_file = os.path.join(output_dir, f"non_streaming_1m_{timestamp}.txt")
    csv_file = os.path.join(output_dir, f"non_streaming_1m_{timestamp}.csv")

    lines = []
    lines.append("非流式实验 (streaming_mode=False) - ml-1m")
    lines.append(f"dataset={dataset}, L=128, epochs=150, time={timestamp}")
    lines.append("")
