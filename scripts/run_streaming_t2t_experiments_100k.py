...
# -*- coding: utf-8 -*-
"""
Test-Then-Train 流式实验脚本 - ml-100k
依次运行 GDN、MoRec、NestRec、DamRec、FroRec，输出格式与 non_streaming 一致。
配置: dataset=ml-100k, L=50, streaming_test_ratio=0.1

用法: python scripts/run_streaming_t2t_experiments_100k.py
"""
...
def run_single_t2t(model_key, dataset="ml-100k", show_progress=True):
...
def main():
    ...
    out_file = os.path.join(output_dir, f"streaming_t2t_100k_{timestamp}.txt")
    csv_file = os.path.join(output_dir, f"streaming_t2t_100k_{timestamp}.csv")

    lines = []
    lines.append("Test-Then-Train 流式实验 (streaming_t2t=True) - ml-100k")
    lines.append(f"dataset={dataset}, L=50, streaming_test_ratio=0.1, time={timestamp}")
    lines.append("")
    ...
    table = "\n".join(lines)
    print("\n" + "=" * 70)
    print("RESULTS (streaming T2T - ml-100k)")
    print("=" * 70)
    print(table)
