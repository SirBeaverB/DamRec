...
# -*- coding: utf-8 -*-
"""
每模型独立预训练 + state_dump + 流式 T2T（yelp2018 80/20 时间切）。

目的：把 ml-1m 上漂到 popularity 天花板的流式实验迁到 yelp2018，验证：
  (1) 离线评分高的 Fro/GDN 在 yelp2018 streaming T2T 下能否超过 popularity baseline
  (2) Fro vs GDN 差距是否跨数据集稳定

与 ml-1m 版的区别：
  - dataset 默认 yelp2018-pretrain / yelp2018-t2t；可用 --data_tag 指向
    yelp2018-pretrain-{tag} / yelp2018-t2t-{tag}（与 prepare 的 --data_tag 一致）
  - 默认只跑 GDN / Adam / Fro 三模型（已去 perN，不再扫 N）
  - 预训练 checkpoint 与 state 落盘到 saved/per_model_pretrain_yelp2018[_tag]_L{L}/

前置：
  python scripts/prepare_yelp2018_80_20_split.py    # 首次运行
  前提是 dataset/yelp2018/yelp2018.inter 已存在（RecBole atomic 格式）
...
Usage:
  ...
  # 多 seed
  python scripts/run_per_model_pretrain_t2t_yelp2018.py --seeds 2020,2021,2022

  # 改 L
  python scripts/run_per_model_pretrain_t2t_yelp2018.py -L 64

  # 断点续跑
  python scripts/run_per_model_pretrain_t2t_yelp2018.py --skip_pretrain
  python scripts/run_per_model_pretrain_t2t_yelp2018.py --skip_pretrain --skip_dump

  # 单模型调试
  CUDA_VISIBLE_DEVICES=0 python scripts/run_per_model_pretrain_t2t_yelp2018.py \\
...
# 由 _configure_data_paths() 根据 --data_tag 填写；无 tag 时为 yelp2018-pretrain / yelp2018-t2t
DATA_TAG = None
PRETRAIN_DATASET = "yelp2018-pretrain"
T2T_DATASET = "yelp2018-t2t"
...
def _configure_data_paths(data_tag=None):
    """与 prepare_yelp2018_80_20_split.py 的 --data_tag 命名一致。"""
...
def _saved_subdir_prefix():
    """saved/ 下目录名：per_model_pretrain_yelp2018 或 per_model_pretrain_yelp2018_u5000。"""
...
def _ensure_yelp_split():
    ...
    if not os.path.isfile(raw):
        raise SystemExit(
            f"未找到 {raw}。请先准备 RecBole atomic 格式的 yelp2018.inter "
            f"(user_id, item_id, rating, timestamp 四列 tab 分隔) 后重试。"
        )
    if DATA_TAG:
        raise SystemExit(
            f"未找到带 data_tag 的切分数据:\n  {PRETRAIN_INTER}\n  {T2T_INTER}\n"
            f"请先执行（标签需与当前 --data_tag 一致），例如:\n"
            f"  python scripts/prepare_yelp2018_80_20_split.py --data_tag {DATA_TAG} --max_users 5000 --seed 42\n"
        )
    print(f"[ensure] 80/20 切分不存在，运行 prepare_yelp2018_80_20_split.py ...")
    subprocess.run(
...
def _set_yelp_overrides(pt1m, t2t_lr=None, epochs=None, seed=None):
    """把 pt1m 模块的 PRETRAIN/T2T OVERRIDES 改成 yelp2018 版本。"""
...
def _patch_ensure_split(pt1m):
    ...
    pt1m._ensure_split = _ensure_yelp_split
...
def _worker_main(args):
    ...
    seed = int(args.seed)
    _set_yelp_overrides(pt1m, t2t_lr=args.t2t_lr, epochs=args.epochs, seed=seed)
...
def _orchestrate(args):
    ...
    try:
        ...
        with open(manifest, "w", encoding="utf-8") as mf:
            ...
            mf.write(f"data_tag  = {DATA_TAG!r}  (None=默认 yelp2018-pretrain / yelp2018-t2t)\n")
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "与 prepare_yelp2018_80_20_split.py 的 --data_tag 一致："
            "使用 dataset/yelp2018-pretrain-{tag}/ 与 yelp2018-t2t-{tag}/；"
            "checkpoint 在 saved/per_model_pretrain_yelp2018_{tag}_L{L}_s{seed}/。默认无 tag=全量目录"
        ),
    )
